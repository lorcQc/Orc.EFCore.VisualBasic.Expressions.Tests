...
Public Class EF6Tests
    ...
    Function GetContext() As EF6BloggingContext
        Return _Fixture.CreateEF6Context
    End Function
    ...
    Public Sub NullableValueComparaison_QuerySyntaxWork()
        Using context = GetContext()
            ...
            Dim Result = Query.ToList()
            Assert.Single(Result)
            Assert.DoesNotContain("COALESCE", Query.ToString())
        End Using
    End Sub
    ...
    Private Sub Log(query As IQueryable)
        _Output.WriteLine("Query : " & Environment.NewLine & "{0}", query.ToString())
    End Sub
